...
runner._pytest_command = lambda: {command}
...
def test_windows_timeout_terminates_parent_and_descendant(
    ...
    monkeypatch.setattr(
        runner,
        "_pytest_command",
        lambda: _tree_command(pid_path, leader_exits=False),
    )

    assert runner.run_merge(budget_seconds=6.0) == runner.TIMEOUT_EXIT_CODE
    _wait_for_path(pid_path)
    _wait_for_process_exit(int(pid_path.read_text(encoding="ascii")))
...
def test_windows_normal_leader_exit_cleans_descendant_and_fails_gate(
    ...
    monkeypatch.setattr(
        runner,
        "_pytest_command",
        lambda: _tree_command(pid_path, leader_exits=True),
    )

    assert runner.run_merge(budget_seconds=10.0) == runner.TERMINATION_FAILED_EXIT_CODE
    _wait_for_path(pid_path)
    _wait_for_process_exit(int(pid_path.read_text(encoding="ascii")))
...
sys.modules[spec.name] = runner
spec.loader.exec_module(runner)
runner._pytest_command = lambda: (
    sys.executable,
    "-c",
    "import sys; assert sys.prefix != sys.base_prefix",
)
raise SystemExit(runner.run_merge(budget_seconds=10.0))
"""
